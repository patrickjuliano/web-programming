...
async function update(id, name, genre, website, recordLabel, bandMembers, yearFormed) {
    if (!id || !name || !genre || !website || !recordLabel || !bandMembers || !yearFormed) throw 'Input not supplied or undefined';

    if (typeof id !== 'string' || typeof name !== 'string' || typeof website !== 'string' || typeof recordLabel !== 'string') throw 'Input is not a string';
    id = id.trim();
    name = name.trim();
    website = website.trim();
    recordLabel = recordLabel.trim();
    if (id.length === 0 || name.length === 0 || website.length === 0 || recordLabel.length === 0) throw 'Strings must not be empty';
    ...
    if (!updateInfo.matchedCount && !updateInfo.modifiedCount) throw 'Could not update band';
